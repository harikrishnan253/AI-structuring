"""
STAGE 2: Gemini 2.5 Pro Classification
- Prompt Builder
...
class GeminiClassifier:
    ...
    def __init__(
        ...
        Args:
            ...
            model_name: Primary Gemini model (default: flash-lite)
            fallback_model_name: Fallback model for low-confidence items (default: flash)
        ...
        # Primary model (Flash-Lite - fast and cheap)
        
        # Set default model names if None
        if model_name is None:
            model_name = "gemini-2.5-flash"
            logger.info("Using default model: gemini-2.5-flash")
        if fallback_model_name is None:
            fallback_model_name = "gemini-2.5-flash"
            logger.info("Using default fallback model: gemini-2.5-flash")
        ...
        # Fallback model (Flash - higher quality)
        ...
        if enable_fallback:
            ...
            fallback_prompt = self._get_fallback_system_prompt()
        ...
        # Token usage tracking (separate for primary and fallback)
...
{formatted_paragraphs}"""
    ...
    def classify(
        ...
        Validates results against zone constraints.
...
[{{"id": ID, "tag": "STYLE", "confidence": 85-99, "reasoning": "brief reason"}}]"""
    ...
    def _call_fallback_model(self, prompt: str, expected_count: int) -> list[dict]:
        ...
        response = self.fallback_model.generate_content(
            prompt,
            request_options={"timeout": 60}  # Shorter timeout for focused calls
        )
        ...
        if hasattr(response, 'usage_metadata'):
            ...
            self.fallback_output_tokens += output_tokens + thinking_tokens
            
            logger.debug(f"Flash fallback tokens - Input: {input_tokens}, Output: {output_tokens + thinking_tokens}")
    ...
    def _classify_chunk(
        ...
        """
        Classify a chunk of paragraphs with retry logic.
        """
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                # Log token usage - IMPORTANT: Account for thinking tokens in Gemini 2.5
                if hasattr(response, 'usage_metadata'):
                    usage = response.usage_metadata
                    input_tokens = getattr(usage, 'prompt_token_count', 0)
                    output_tokens = getattr(usage, 'candidates_token_count', 0)
                    ...
                    # Calculate thinking tokens (Gemini 2.5 feature)
                    # Thinking tokens = total - input - output
                    # These are billed as output tokens but not shown in candidates_token_count
                    thinking_tokens = max(0, total_tokens - input_tokens - output_tokens)
                    
                    # For billing purposes, add thinking tokens to output
                    # (Google bills thinking tokens at output token rates)
                    ...
                    # Accumulate total usage (use billable tokens for accurate cost tracking)
                    self.total_input_tokens += input_tokens
                    self.total_output_tokens += billable_output_tokens  # Include thinking tokens
                    self.total_tokens += total_tokens
                    
                    logger.info(f"Token Usage - Input: {input_tokens:,}, Output: {output_tokens:,}, Thinking: {thinking_tokens:,}, Total: {total_tokens:,}")
                    if thinking_tokens > 0:
                        logger.info(f"  (Billable output = {billable_output_tokens:,} including thinking tokens)")
                    logger.info(f"Cumulative Tokens - Input: {self.total_input_tokens:,}, Output: {self.total_output_tokens:,}, Total: {self.total_tokens:,}")
                else:
        ...
        logger.error(f"All {self.max_retries} retries failed for Gemini API")
    ...
    def get_token_usage(self) -> dict:
        ...
        return {
            ...
            'last_call': self._last_token_usage,
            ...
            # Combined totals
            'combined_input_tokens': self.total_input_tokens + self.fallback_input_tokens,
            'combined_output_tokens': self.total_output_tokens + self.fallback_output_tokens,
        }
