...
def detect_list_properties(paragraph):
    """
    Detect if paragraph is part of a list and get list properties.
    
    Returns:
        dict with 'is_list', 'list_type', 'level', 'num_id'
    """
    result = {
        'is_list': False,
        'list_type': None,
    ...
    # Check Word numbering properties  
    ...
    if numId is not None:
        result['num_id'] = int(numId.get(qn('w:val')))
    
    # Get level (ilvl)
    ilvl = numPr.find(qn('w:ilvl'))
    if ilvl is not None:
        result['level'] = int(ilvl.get(qn('w:val')))
    
    # Determine if bullet or number by checking text
    text_stripped = paragraph.text.lstrip()
    
    import re
    number_patterns = [
        r'^\d+[\.)\s]',  # 1. or 1) or 1 space
        r'^[A-Z][\.)\s]',  # A. or A) or A space
        r'^[a-z][\.)\s]',  # a. or a) or a space
        r'^[ivxlcdm]+[\.)\s]',  # Roman numerals
    ]
    
    is_numbered = any(re.match(pattern, text_stripped, re.IGNORECASE) for pattern in number_patterns)
    
    result['list_type'] = 'number' if is_numbered else 'bullet'
...
class DocumentIngestion:
    ...
    def extract_paragraphs(self, docx_path: str | Path) -> list[dict]:
        ...
        for para in doc.paragraphs:
            ...
            list_props = detect_list_properties(para)
            metadata["list_properties"] = list_props
            
            # Add list hint to text for AI
            if list_props["is_list"]:
                list_type = list_props["list_type"]
                level = list_props["level"]
                hint = f"[{list_type.upper()}_LIST_L{level}] "
                text = hint + text
