...
class DocumentReconstructor:
    ...
    def __init__(self, output_dir: str | Path):
        ...
        self.style_handler = get_global_handler()
        logger.info(f"Style handler initialized with {len(self.style_handler.official_tags)} official tags")
    ...
    def apply_styles(
        ...
        for para in doc.paragraphs:
            ...
            if clf:
                                # Try to extract tag from text first
                text_tag = self.style_handler.extract_tag_from_text(text)
                if text_tag:
                    # Tag found in text - use it (after normalization)
                    tag = self.style_handler.normalize_tag(text_tag)
                    logger.debug(f"Extracted tag '{text_tag}' from text (normalized to '{tag}')")
                else:
                    # Use AI classification
                    tag = self.style_handler.normalize_tag(clf["tag"])
        ...
        for table in doc.tables:
            for row in table.rows:
                for cell in row.cells:
                    ...
                    clf = clf_lookup.get(para_id)
                    if clf:
                                                # Extract or normalize tag
                        text_tag = self.style_handler.extract_tag_from_text(cell_text)
                        if text_tag:
                            tag = self.style_handler.normalize_tag(text_tag)
                        else:
                            tag = self.style_handler.normalize_tag(clf["tag"])
